...
void wywolanieWyjatku_div0() {
	printf("Próbuję wykonać dzielenie przez 0... ");
	asm("fldz\nfldpi\nfdivp\n");
	printf("Gotowe.\n\n");
}

void wywolanieWyjatku_so() {
	printf("Próbuję wywołać przepełnienie stosu... ");
	asm("fldz\nfldz\nfldz\nfldz\nfldz\nfldz\nfldz\nfldz\nfldz\n");
	printf("Gotowe.\n\n");
}
...
int main() {
	...
	short stan = asm_stanRejestruKontrolnego(), stanFPU = 0;
	printf("Stan: %#04x.\n", stan);
	tlumaczenieFlagKontrolnych(stan);
	
	wywolanieWyjatku_div0();
	stanFPU	= asm_stanFPU();

	printf("Stan FPU:\n");
	tlumaczenieFlagStanu(stanFPU);
